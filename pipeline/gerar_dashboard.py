...
<title>Radar de Editais — Big Data · Gestão em Saúde (PNCP)</title>
<style>
:root{--navy:#0B3B5A;--navy2:#125E8A;--teal:#0E7C7B;--bg:#f0f2f5;--card:#ffffff;--ok:#2D6A4F;--warn:#9B2226;--amber:#B45309;--muted:#5b6b7a}
...
tbody tr.monit{background:#fff8ec}
tbody tr.monit:hover{background:#fdf1d8}
...
.b-eixo{background:#e6f0fa;color:#125E8A}
.b-disp{background:#e2f5ea;color:var(--ok)}
.b-monit{background:#fbe7cf;color:var(--amber)}
.star{color:#E9A400;font-weight:bold}
...
<header>
<h1>🩺 Radar de Editais — Big Data · Gestão em Saúde</h1>
<div class="sub">Fonte: PNCP (dados abertos) · Brasil inteiro · gerado em __GERADO_EM__ · build __BUILD__</div>
</header>
<div class="wrap">
  <div class="cards">
    <div class="card"><div class="n" id="cTotal">0</div><div class="l">Editais captados em aberto</div></div>
    <div class="card"><div class="n" id="cValor">R$ 0</div><div class="l">Valor total estimado</div></div>
    <div class="card"><div class="n" id="cPrio">0</div><div class="l">⭐ Prioritários (as 3 frentes)</div></div>
  ...
  <div class="filters">
    <div><label>Foco</label><select id="fFoco">
      <option value="">Todos</option>
      <option value="disputar">Disputar</option>
      <option value="prio">⭐ Prioritários</option>
      <option value="monitorar">Monitorar (não disputar)</option>
    </select></div>
    <div><label>Eixo / CNAE</label><select id="fEixo"><option value="">Todos</option></select></div>
    ...
    <div><label>Buscar (órgão, município, objeto)</label><input type="text" id="fBusca" placeholder="ex.: telemedicina, RPPS, ponto..."></div>
  </div>
...
<footer>Etiquetagem por eixo/CNAE e classificação de fonte de recurso são heurísticas (baseadas no texto do objeto) — sempre confirmar no edital antes de decidir. Itens marcados <span class="badge b-monit">Monitorar</span> são do mercado de operadoras/planos (acompanhar, não disputar sem atestado de operação). Dados: Portal Nacional de Contratações Públicas (PNCP).</footer>
<script>
...
function eixosBadges(i){ return (i.eixos||[]).map((e,idx)=>`<span class="badge b-eixo" title="${i.cnaes&&i.cnaes[idx]?('CNAE '+i.cnaes[idx]):''}">${e}</span>`).join(''); }
...
const eixos = [...new Set(itens.flatMap(i=>i.eixos||[]))].sort();
const selEixo = document.getElementById('fEixo');
eixos.forEach(e=>{ const o=document.createElement('option'); o.value=e; o.textContent=e; selEixo.appendChild(o); });
...
function render(){
  const eFoco = document.getElementById('fFoco').value;
  const eEixo = document.getElementById('fEixo').value;
  ...
  let filtrados = itens.filter(i=>{
    if(eFoco==='disputar' && i.categoria!=='disputar') return false;
    if(eFoco==='monitorar' && i.categoria!=='monitorar') return false;
    if(eFoco==='prio' && !i.prioritario) return false;
    if(eEixo && !(i.eixos||[]).includes(eEixo)) return false;
    ...
    if(eBusca){
      const alvo = (i.orgao+' '+i.municipio+' '+i.objeto+' '+(i.eixos||[]).join(' ')).toLowerCase();
  ...
  document.getElementById('countInfo').textContent = `Mostrando ${filtrados.length} de ${itens.length} editais captados em aberto no Brasil.`;
  ...
  document.getElementById('cPrio').textContent = filtrados.filter(i=>i.prioritario).length;
  ...
  tbody.innerHTML = filtrados.slice(0,500).map(i=>{
    ...
    const monit = i.categoria==='monitorar';
    return `<tr class="${monit?'monit':''}">
      <td>${i.prioritario?'<span class="star" title="Prioritário (as 3 frentes)">⭐</span> ':''}${fmtData(i.dataEncerramentoProposta)}${urgente?' <span class="badge b-ata-n">urgente</span>':''}<div class="small">Emitido: ${fmtDataHora(i.dataPublicacaoPncp)}</div></td>
      ...
      <td>${eixosBadges(i)}${monit?'<span class="badge b-monit">Monitorar</span>':'<span class="badge b-disp">Disputar</span>'}</td>
      <td>${i.modalidade||'—'}</td>
  ...
  if(filtrados.length>500){
    tbody.innerHTML += `<tr><td colspan="11" class="small" style="text-align:center;padding:12px">Mostrando os 500 primeiros (use os filtros para refinar).</td></tr>`;
  }
}
...
['fFoco','fEixo','fEsfera','fUf','fAta','fFonte'].forEach(id=>document.getElementById(id).addEventListener('change',render));
